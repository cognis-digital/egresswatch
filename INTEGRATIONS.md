...
**egresswatch** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `egresswatch` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
egresswatch ... --format json | cognis-connect emit --to sigma   # Sigma rules
egresswatch ... --format json | cognis-connect emit --to splunk --url $URL --token $TOK   # Splunk HEC
egresswatch ... --format json | cognis-connect emit --to elastic --url $URL --token $TOK   # Elastic _bulk
egresswatch ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `egresswatch` outputs:

```python
from cognis_connect import normalize, sigma
findings = [normalize(rec, source="egresswatch") for rec in records]   # records = your JSON output
