"""Native cognis-connect emit for egresswatch — forward findings to any platform.

Maps egresswatch's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    egresswatch ... --format json | egresswatch-emit --to stix
    egresswatch-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('event', {}).get('name', 'Unknown Event')
        out['severity'] = rec.get('event', {}).get('severity', 'info')
        out['type'] = rec.get('event', {}).get('type', 'unknown')
        out['description'] = rec.get('event', {}).get('message', 'No description')
