...
TOOL_VERSION = "0.1.0"
RULES = [{'id': 'EW-NEW-001', 'severity': 'high', 'weight': 2.5, 'title': 'EGRESS_TO_NEW_DEST', 'when': {'first_seen': True, 'port': ['22', '443', '8443']}, 'remediation': 'Verify; add to allowlist or block.'}, {'id': 'EW-DGA-001', 'severity': 'high', 'weight': 2.5, 'title': 'DGA_LIKE_HOSTNAME', 'when': {'dga_score': '^([7-9]|10)'}, 'remediation': 'Block; hunt for malware on source host.'}]
        ...
        for rule in RULES:
            if _match(item, rule):
                result.add(Finding(
                    ...
                    location=str(target),
                    remediation=rule.get("remediation",""), category="egress",
                    metadata=item,
                ))
